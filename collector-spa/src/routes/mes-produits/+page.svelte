<script lang="ts">
	...
	import { formatPrice, formatPromotionValue } from '$lib/promotions';
	import {
	...
	type SellerSection = 'dashboard' | 'products' | 'promotions';

	const sellerSections = [
		{ id: 'dashboard' as const, title: 'Dashboard', description: 'Vue rapide et indicateurs' },
		{ id: 'products' as const, title: 'Produits', description: 'Tes annonces en vente' },
		{ id: 'promotions' as const, title: 'Promotions', description: 'Tes remises actives' }
	];

	let { data, form }: { data: PageData; form: ActionData | null } = $props();

	let activeSection = $state<SellerSection>('dashboard');
	...
	const dashboardCards = $derived.by(() => [
		{
			...
			description: 'Total des ventes payees'
		},
		{
			label: 'Ventes',
			value: data.stats.totalSales,
			description: 'Unites vendues et payees'
		},
		{
			label: 'Produits',
			value: data.stats.productCount,
			description: 'Annonces publiees'
		},
		{
			label: 'Promotions actives',
			value: data.promotions.filter((promotion) => promotion.isActive).length,
			description: `${data.promotions.length} promotion(s) au total`
		}
	]);
	...
	const countPromotionsForProduct = (productId: number) =>
		data.promotions.filter((promotion) => promotion.productIds.includes(productId)).length;
...
<section class="space-y-6">
	...
	<div class="theme-panel p-8">
		<div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
			<div>
				<span class="theme-pill">Espace vendeur</span>
				<h1 class="theme-title mt-4 text-4xl font-black">Mon dashboard</h1>
				<p class="theme-copy mt-3 max-w-2xl">
					Suis tes ventes, gere tes produits et tes promotions depuis un seul endroit.
				</p>
			</div>

			<div class="admin-tabs">
				{#each sellerSections as section (section.id)}
					<button
						type="button"
						class="admin-tab"
			...
			{#if data.products.length === 0}
				<div class="theme-empty theme-panel">
					...
					<p class="theme-copy mt-3">
						Publie ta premiere annonce pour apparaitre dans le catalogue.
					</p>
					<button
						type="button"
						class="theme-button theme-button-primary mt-5"
			...
			<div class="theme-panel p-8">
				<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<h2 class="theme-title mt-3 text-3xl font-black">Mes produits</h2>
						<p class="theme-copy mt-2">Pilote tes annonces, ton stock et tes promotions.</p>
					</div>
					<button
						type="button"
						...
						<tbody>
							{#each data.products as product (product.id)}
								<tr>
									...
									<td class="theme-copy text-sm">{product.stock}</td>
			...
			<div class="theme-panel p-8">
				<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<h2 class="theme-title mt-3 text-3xl font-black">Mes promotions</h2>
						<p class="theme-copy mt-2">
							Configure des remises sur tes propres produits, en pourcentage ou en montant fixe.
						</p>
					</div>
					<button
						type="button"
				...
				{#if data.products.length === 0}
					<p class="theme-copy mt-3 text-sm">
						Ajoute d'abord un produit pour pouvoir creer une promotion.
					</p>
				{/if}
			</div>
	...
	<div class="theme-overlay fixed inset-0 z-40 flex items-center justify-center px-4 py-8">
		<div class="theme-modal w-full max-w-xl p-8">
			...
			>
				...
				<div>
					<label for="create-stock" class="theme-label">Stock</label>
					<input
						id="create-stock"
						name="stock"
						type="number"
						...
						step="1"
						required
						value={form?.action === 'create-product' ? (form.values?.stock ?? '1') : '1'}
						class="theme-input"
					/>
				</div>
				<div>
		...
		<div class="theme-modal w-full max-w-3xl p-8">
			<div class="modal-header">
				<div>
					...
					<p class="theme-copy mt-2 text-sm">Choisis une remise et les produits concernes.</p>
				</div>
				<button
					type="button"
					class="theme-icon-button theme-close"
				...
				<div class="theme-card p-4">
					<div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
						<div>
							...
							<p class="theme-copy mt-1 text-sm">Selectionne un ou plusieurs de tes produits.</p>
						</div>
					...
					<div class="promotion-product-grid mt-4">
						{#each data.products as product (product.id)}
							<label class="promotion-product-card">
								<input
									type="checkbox"
									name="product_ids"
									value={product.id}
		...
		<div class="theme-modal w-full max-w-3xl p-8">
			<div class="modal-header">
				<div>
					...
					<p class="theme-copy mt-2 text-sm">Ajuste la remise et les produits cibles si besoin.</p>
				</div>
				<button
					type="button"
					class="theme-icon-button theme-close"
			...
			<form method="POST" action="?/updatePromotion" class="mt-6 space-y-4">
				...
				<label class="checkbox-card">
					<input type="checkbox" name="is_active" value="true" bind:checked={editPromotionActive} />
					<span class="theme-title text-sm font-medium">Promotion active</span>
				</label>
